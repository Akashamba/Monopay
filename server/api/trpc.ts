...
export const createTRPCContext = async (opts: { headers: Headers }) => {
  console.log("Creating tRPC context with headers:", opts.headers);
  const session = await auth.api.getSession({
    headers: await opts.headers, // you need to pass the headers object.
  });
...
export const protectedProcedure = t.procedure.use(({ ctx, next }) => {
  console.log("Protected procedure middleware - checking auth...");
  if (!ctx.user) {
    console.error("No user found in context");
    throw new TRPCError({ code: "UNAUTHORIZED" });
  }
  console.log("User authorized:", ctx.user);
  return next({
    ctx: {
      ...ctx,
      user: ctx.user,
